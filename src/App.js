...
function Header({showForm, setShowForm}) {
  return <header className="header">
    ...
    <button className="btn btn-large btn-open" onClick={() => setShowForm((show) => !show)}>{showForm ? "Close" : "Share a post"}</button>
  </header>
}
...
function FactList({facts,setFacts}) {
  ...
  if (facts.length === 0) {
    return <p className='message'>There is no facts right now, just create one! ✌️</p>
  }
  ...
  return (
    <section>
      <ul className='fact-lists'>
        {facts.map((fact) => (
          <Fact key={fact.name} fact={fact} setFacts={setFacts}/>
        ))}
      </ul>
      <p>There are {facts.length} facts here, Add your own!</p>
    </section>
  )
}
...
function Fact({ fact, setFacts }) {
  ...
  return <li className="fact">
    <p>
      {isDisputed ? <p className='disputed'>[⛔️DISPUTED]</p> : null}
      {fact.text}
